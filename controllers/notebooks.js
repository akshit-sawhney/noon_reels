...
async function getNotebooksByNameAndUserId(userId, notebookName) {
  const querySpec = {
    query: `SELECT * FROM ${process.env.AZURE_COSMOS_NOTEBOOKS_CONTAINER_ID} nb WHERE  nb.user_id = @userId and nb.notebook_name = @notebookName`,
    parameters: [
      {
        name: "@userId",
        value: userId
...
module.exports = { createNotebook, getNoteBooksByUserId, getNotebooksByNameAndUserId };